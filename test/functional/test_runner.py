...
BASE_SCRIPTS = [
    ...
    "feature_blockchain_progression.py",
    "feature_rewards.py",
    "wallet_basic.py",
    "mempool_limit.py",
    "feature_assets.py",
...
def main():
    ...
    parser.add_argument(
        "--filter", metavar="", help="Filter scripts to run by regular expression."
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="How many test scripts to run in parallel. Default=."
        + str(get_cpu_count()),
    )
    parser.add_argument(
        "--keepcache",
        action="store_true",
        help="The default behavior is to flush the cache directory on startup. --keepcache retains the cache from the previous test-run.",
    )
    parser.add_argument(
        "--list", action="store_true", help="Print list of tests and exit."
    )
    parser.add_argument(
        "--loop",
        type=int,
        metavar="n",
        default=1,
        help="Run(loop) the tests n number of times.",
    )
    parser.add_argument(
        "--onlyextended", action="store_true", help="Run only the extended test suite."
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
    ...
    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/clore-ai/clore/commit/d52802551752140cf41f0d9a225a43e84404d3e9
    ...
    enable_clore_blockchaind = config["components"].getboolean(
        "ENABLE_CLORE_BLOCKCHAIND"
    )
    if not (enable_wallet and enable_cli and enable_clore_blockchaind):
        print(
            "No functional tests to run. Wallet, utils, and clore_blockchaind must all be enabled"
        )
        print(
            "Rerun `configure` with --enable-wallet, --with-cli and --with-daemon and rerun make"
        )
        sys.exit(0)
...
def run_tests(
    ...
    try:
        if subprocess.check_output(["pidof", "clore_blockchaind"]) is not None:
            print(
                "%sWARNING!%s There is already a clore_blockchaind process running on this system. Tests may fail unexpectedly due to resource contention!"
                % (BOLD[1], BOLD[0])
            )
    ...
    for _ in range(test_count):
        ...
        done_str = "{}/{} - {}{}{}".format(
            _ + 1, test_count, BOLD[1], test_result.name, BOLD[0]
        )
    ...
    all_passed = (
        all(map(lambda test_res: test_res.was_successful, test_results))
        and coverage_passed
    )
...
class TestHandler:
    ...
    def get_next(self):
        while self.num_running < self.num_jobs and self.test_list:
            ...
            test_dir = "{}/{}_{}".format(
                self.tmpdir, re.sub(".py$", "", test_argv[0]), port_seed
            )
...
def check_script_prefixes():
    ...
    if len(bad_script_names) < expected_violation_count:
        ...
        print(
            "Consider reducing expected_violation_count from %d to %d"
            % (expected_violation_count, len(bad_script_names))
        )
    elif len(bad_script_names) > expected_violation_count:
        print(
            "WARNING: %d tests not meeting naming conventions.  Please rename with allowed prefix. (expected %d):"
            % (len(bad_script_names), expected_violation_count)
        )
        print("  %s" % ("\n  ".join(sorted(bad_script_names))))
        assert len(bad_script_names) <= expected_violation_count + leeway, (
            "Too many tests not following naming convention! (%d found, expected: <= %d)"
            % (len(bad_script_names), expected_violation_count)
        )
...
def check_script_list(src_dir):
    ...
    missed_tests = list(
        python_files
        - set(map(lambda x: x.split()[0], ALL_SCRIPTS + NON_SCRIPTS + SKIPPED_TESTS))
    )
    if len(missed_tests) != 0:
        print(
            "%sWARNING!%s The following scripts are not being run:\n%s \nCheck the test lists in test_runner.py."
            % (BOLD[1], BOLD[0], "\n".join(missed_tests))
        )
