...
class TxnMallTest(CloreTestFramework):
    ...
    def run_test(self):
        ...
        # Create two spends using 1 50 CLORE each
        ...
        assert_equal(
            self.nodes[0].getbalance("foo", 0), 121900 + tx1["amount"] + tx1["fee"]
        )
        assert_equal(
            self.nodes[0].getbalance("bar", 0), 2900 + tx2["amount"] + tx2["fee"]
        )
        ...
        if self.options.mine_block:
            ...
            assert_equal(
                self.nodes[1].getbalance("from0"), -(tx1["amount"] + tx2["amount"])
            )
        else:
