...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    // For regtest, skip proof of work check entirely to avoid genesis block issues
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, const CChainParams& chainparams, CAssetsCache* assetsCache = nullptr, bool fJustCheck = false, bool ignoreAddressIndex = false)
{
    ...
    // CLORE: Phased PoS transition validation
    ...
    // Phase 3: Pure PoS - PoW mining disabled, only PoS blocks accepted
    if (isPosOnlyActive) {
        if (!isPoSBlock) {
            return state.DoS(100, error("ConnectBlock(): PoW block rejected - PoS-only phase active"),
                REJECT_INVALID, "pow-disabled");
        }
        LogPrint(BCLog::STAKING, "ConnectBlock(): PoS-only validation passed for block %s\n",
    ...
    // Both PoW and PoS blocks are allowed during infrastructure phases
        ...
        LogPrint(BCLog::STAKING, "ConnectBlock(): Infrastructure phase - block type: %s\n",
            isPoSBlock ? "PoS" : "PoW");
    }
    // Phase 0: Pure PoW (before any PoS infrastructure)
    else {
        if (isPoSBlock) {
            return state.DoS(100, error("ConnectBlock(): PoS block rejected - PoS not yet active"),
                REJECT_INVALID, "pos-too-early");
        }
        LogPrint(BCLog::STAKING, "ConnectBlock(): Pure PoW validation for block %s\n",
            block.GetHash().ToString());
    }
    ...
    try {
        {
            ...
            if (fDoFullFlush) {
                ...
                if (!CheckDiskSpace((48 * 2 * 2 * pcoinsTip->GetCacheSize()))) /** CLORE START */ /** CLORE END */
                    return state.Error("out of disk space");
...
{
    // For regtest, skip proof of work checks entirely
    if (consensusParams.fPowNoRetargeting) {
        return true;
    }
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check proof of work
    LogPrintf("DEBUG: ContextualCheckBlockHeader about to check proof of work\n");
    ...
    LogPrintf("DEBUG: ContextualCheckBlockHeader about to call GetMedianTimePast\n");
    LogPrintf("DEBUG: ContextualCheckBlockHeader pindexPrev=%p\n", pindexPrev);
    if (pindexPrev) {
        LogPrintf("DEBUG: ContextualCheckBlockHeader pindexPrev->nHeight=%d\n", pindexPrev->nHeight);
    }
    LogPrintf("DEBUG: ContextualCheckBlockHeader calling GetMedianTimePast\n");
    int64_t medianTimePast = pindexPrev->GetMedianTimePast();
    LogPrintf("DEBUG: ContextualCheckBlockHeader GetMedianTimePast returned %ld\n", medianTimePast);
    LogPrintf("DEBUG: ContextualCheckBlockHeader block.GetBlockTime()=%lu\n", block.GetBlockTime());
    if (block.GetBlockTime() <= medianTimePast)
        return state.Invalid(false, REJECT_INVALID, "time-too-old", "block's timestamp is too early");

    // Check timestamp
    LogPrintf("DEBUG: ContextualCheckBlockHeader about to check timestamp\n");
    if (IsDGWActive(pindexPrev->nHeight + 1)) {
        LogPrintf("DEBUG: ContextualCheckBlockHeader DGW is active, checking DGW time\n");
        if (block.GetBlockTime() > nAdjustedTime + MAX_FUTURE_BLOCK_TIME_DGW)
            return state.Invalid(false, REJECT_INVALID, "time-too-new", "block timestamp too far in the future");
    } else {
        LogPrintf("DEBUG: ContextualCheckBlockHeader DGW is NOT active, checking normal time\n");
        LogPrintf("DEBUG: ContextualCheckBlockHeader block.GetBlockTime()=%lu\n", block.GetBlockTime());
        LogPrintf("DEBUG: ContextualCheckBlockHeader nAdjustedTime=%ld\n", nAdjustedTime);
        LogPrintf("DEBUG: ContextualCheckBlockHeader MAX_FUTURE_BLOCK_TIME=%d\n", MAX_FUTURE_BLOCK_TIME);
        LogPrintf("DEBUG: ContextualCheckBlockHeader about to compare times\n");
        if (block.GetBlockTime() > nAdjustedTime + MAX_FUTURE_BLOCK_TIME)
            return state.Invalid(false, REJECT_INVALID, "time-too-new", "block timestamp too far in the future");
        LogPrintf("DEBUG: ContextualCheckBlockHeader timestamp check passed\n");
    }
...
static bool AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fFromLoad = false)
{
    ...
    if (!CheckBlock(block, state, chainparams.GetConsensus(), true, true) ||
        !ContextualCheckBlock(block, state, chainparams.GetConsensus(), pindex->pprev, nullptr)) {
...
{
    LogPrintf("DEBUG: TestBlockValidity start\n");
    AssertLockHeld(cs_main);
    assert(pindexPrev && pindexPrev == chainActive.Tip());
    LogPrintf("DEBUG: TestBlockValidity creating CCoinsViewCache\n");
    CCoinsViewCache viewNew(pcoinsTip);
    LogPrintf("DEBUG: TestBlockValidity creating CBlockIndex\n");
    CBlockIndex indexDummy(block);
    indexDummy.pprev = pindexPrev;
    indexDummy.nHeight = pindexPrev->nHeight + 1;

    LogPrintf("DEBUG: TestBlockValidity calling ContextualCheckBlockHeader\n");
    if (!ContextualCheckBlockHeader(block, state, chainparams, pindexPrev, GetAdjustedTime()))
        return error("%s: Consensus::ContextualCheckBlockHeader: %s", __func__, FormatStateMessage(state));
    LogPrintf("DEBUG: TestBlockValidity calling CheckBlock\n");
    if (!CheckBlock(block, state, chainparams.GetConsensus(), fCheckPOW, fCheckMerkleRoot))
        return error("%s: Consensus::CheckBlock: %s", __func__, FormatStateMessage(state));
    LogPrintf("DEBUG: TestBlockValidity calling ContextualCheckBlock\n");
    if (!ContextualCheckBlock(block, state, chainparams.GetConsensus(), pindexPrev, nullptr))
        return error("%s: Consensus::ContextualCheckBlock: %s", __func__, FormatStateMessage(state));

    LogPrintf("DEBUG: TestBlockValidity creating asset cache\n");
    // Create proper asset cache for ConnectBlock - use empty cache when assets disabled
    auto currentActiveAssetCache = GetCurrentAssetCache();
    CAssetsCache assetsCache(currentActiveAssetCache ? *currentActiveAssetCache : CAssetsCache());

    LogPrintf("DEBUG: TestBlockValidity calling ConnectBlock\n");
    if (!ConnectBlock(block, state, &indexDummy, viewNew, chainparams, &assetsCache, true))
        return error("%s: Consensus::ConnectBlock: %s", __func__, FormatStateMessage(state));
    LogPrintf("DEBUG: TestBlockValidity asserting state valid\n");
    assert(state.IsValid());

    LogPrintf("DEBUG: TestBlockValidity completed successfully\n");
    return true;
}
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    // Skip verification in regtest mode to avoid hang issues
    if (chainparams.NetworkIDString() == "regtest") {
        LogPrintf("Skipping database verification in regtest mode\n");
        return true;
    }
    ...
    auto currentActiveAssetCache = GetCurrentAssetCache();
    CAssetsCache assetCache(currentActiveAssetCache ? *currentActiveAssetCache : CAssetsCache());
    LogPrintf("[0%%]...");
...
{
    LogPrintf("DEBUG: IsDGWActive called with nBlockNumber=%u\n", nBlockNumber);
    LogPrintf("DEBUG: IsDGWActive calling GetParams()\n");
    const CChainParams& params = GetParams();
    LogPrintf("DEBUG: IsDGWActive calling DGWActivationBlock()\n");
    unsigned int activationBlock = params.DGWActivationBlock();
    LogPrintf("DEBUG: IsDGWActive activation block=%u, returning %s\n", activationBlock, (nBlockNumber >= activationBlock) ? "true" : "false");
    return nBlockNumber >= activationBlock;
}
