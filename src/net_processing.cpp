                ...
                if (mi != mapBlockIndex.end()) {
                    ...
                    if (chainActive.Contains(mi->second)) {
                        ...
                        send = mi->second->IsValid(BLOCK_VALID_SCRIPTS) &&
                               StaleBlockRequestAllowed(mi->second, consensusParams) && (chainActive.Height() - (mi->second->nHeight - 1) < GetParams().MaxReorganizationDepth());
                        if (!send) {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::REJECT) {
        ...
        if (IsRip5Active() && nVersion < MESSAGING_RESTRICTED_ASSETS_VERSION) {
            ...
            connman->PushMessage(pfrom, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE, strprintf("Version must be %d or greater", MESSAGING_RESTRICTED_ASSETS_VERSION)));
            pfrom->fDisconnect = true;
            return false;
        }
        ...
        if (AreTransferScriptsSizeDeployed() && nVersion < KAWPOW_VERSION) {
            ...
            connman->PushMessage(pfrom, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE, strprintf("Version must be %d or greater or equal to", KAWPOW_VERSION)));
            pfrom->fDisconnect = true;
            return false;
        }
